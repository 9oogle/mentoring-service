...
public class BookingResponse {
	public record Create(UUID enrollmentId, UUID productId, String productName, long productPrice,
		...
		public static Create of(BookingResult.Create createResult) {
			return new Create(createResult.enrollmentId(), createResult.productId(),
					createResult.productName(), createResult.productPrice(),
					createResult.instructorId(), createResult.instructorName());
		}
	}
	...
	public record Summary(UUID bookingId, String mentoringTitle, String mentorName,
		...
		public static Summary of(BookingResult.Summary result) {
			return new Summary(result.bookingId(), result.mentoringTitle(), result.mentorName(),
					result.menteeName(), result.status(), result.requestedSessions(),
					result.createdAt());
		}
	}
	...
	public record SessionInfo(UUID sessionId, LocalDate sessionDate, LocalTime startTime,
		...
		public static SessionInfo of(BookingResult.SessionInfo result) {
			return new SessionInfo(result.sessionId(), result.sessionDate(), result.startTime(),
					result.endTime(), result.progressStatus());
		}
	}
}
