...
public class MentoringResult {
	...
	public record Detail(UUID mentoringId, String title, String subtitle, String description,
		...
		public static Detail from(Mentoring m) {
			return new Detail(m.getMentoringId()
					.mentoringId(), m.getTitle(), m.getSubtitle(), m.getDescription(),
					new MentorInfo(m.getMentor()
							.getName(), m.getMentor()
							.getField()), new CategoryInfo(m.getMentoringCategory()
					.getName(), m.getMentoringCategory()
					.getCode()), m.getStatus(), m.getFormat(), m.getMentoringType(),
					m.getDuration(), m.getSessionCount(), m.getMaxParticipants(),
					m.isExcludeHolidays(), m.getPrice(), m.getEndDate());
		}
	...
	public record Schedules(List<RepeatPatternDto> repeatPatterns, List<SessionDto> sessions) {
		public static Schedules from(Mentoring m) {
			List<RepeatPatternDto> patterns = m.getRepeatPatterns()
					.stream()
					.map(p -> new RepeatPatternDto(p.getDayOfWeek(), p.getStartTime(),
							p.getEndTime()))
					.toList();
			List<SessionDto> sessions = m.getSessions()
					.stream()
					.map(s -> new SessionDto(s.getSessionDate(), s.getSessionStartTime(),
							s.getSessionEndTime(), s.getStatus()))
					.toList();
	...
	public record Summary(UUID mentoringId, String title, String subtitle, String mentorName,
		...
		public static Summary from(Mentoring m) {
			return new Summary(m.getMentoringId()
					...
					.getName(), m.getMentoringCategory()
					.getName(), m.getMentoringType(), m.getFormat(), m.getPrice(), m.getStatus());
		}
	}
}
