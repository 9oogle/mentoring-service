...
@Import({WebMvcConfig.class, UserContextArgumentResolver.class, MentoringControllerTest.PageResolverConfig.class})
...
class MentoringControllerTest {
    ...
    void getMentoring_returns_200_with_detail() throws Exception {
        UUID mentoringId = UUID.randomUUID();
        MentoringResult.Detail detail = new MentoringResult.Detail(
                mentoringId, TITLE, SUBTITLE, DESCRIPTION,
                ...
                MentoringDuration.MINUTES_60, SESSION_COUNT, MAX_PARTICIPANTS, false, PRICE, null
        );
    ...
    void getMentoring_returns_404_when_not_found() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/mentorings/{mentoringId}", unknownId))
                ...
                .andExpect(jsonPath("$.detail").value(org.hamcrest.Matchers.containsString(unknownId.toString())));
    }
    ...
    void getMentoringSchedules_returns_200_with_schedules() throws Exception {
        ...
        MentoringResult.Schedules schedules = new MentoringResult.Schedules(
                List.of(
                        new MentoringResult.Schedules.RepeatPatternDto(DayOfWeek.MONDAY, START_TIME, END_TIME),
                        new MentoringResult.Schedules.RepeatPatternDto(DayOfWeek.WEDNESDAY, START_TIME, END_TIME)
                ),
                List.of(
                        new MentoringResult.Schedules.SessionDto(SESSION_DATE_1, START_TIME, END_TIME, SessionStatus.AVAILABLE),
                        new MentoringResult.Schedules.SessionDto(SESSION_DATE_2, START_TIME, END_TIME, SessionStatus.AVAILABLE)
                )
        );
    ...
    void searchMentorings_returns_200_with_page() throws Exception {
        UUID mentoringId = UUID.randomUUID();
        MentoringResult.Summary summary = new MentoringResult.Summary(
                mentoringId, TITLE, SUBTITLE, MENTOR_NAME, CATEGORY_NAME,
                MentoringType.ONE_ON_ONE, Format.SINGLE, PRICE, MentoringStatus.INACTIVE
        );
